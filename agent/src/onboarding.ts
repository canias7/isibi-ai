 ...
 *
 * 4-step setup: Welcome → Accessibility → Screen Recording → Done
...
export function registerOnboardingIPC(): void {
  ...
  // Keep for backward compat but no longer used in onboarding UI
...
.ico.ac{background:rgba(236,72,153,.12)}.ico.sc{background:rgba(139,92,246,.12)}
...
<div class="wiz">
<div class="tb"><div class="dots"><div class="dot c" onclick="window.close()"></div><div class="dot m"></div><div class="dot g"></div></div><span class="sl" id="sl">Step 1 of 4</span></div>
<div class="s on" id="s1"><div class="orb"></div><h1>Ghost Mode</h1><p>Your AI agent that controls your computer. Let's set up permissions — takes 30 seconds.</p><button class="btn p" onclick="go(2)">Get Started</button></div>
<div class="s" id="s2"><div class="ico ac">\ud83d\uddb1\ufe0f</div><h2>Accessibility Access</h2><p>Ghost Mode needs to control your mouse and keyboard to interact with apps for you.</p><div id="ab" class="badge wait">\u23f3 Not granted yet</div><div class="br"><button class="btn g" onclick="openAcc()">Open Settings</button><button class="btn p" onclick="nextAcc()">I've Enabled It</button></div><div class="hint">System Settings \u2192 Privacy & Security \u2192 Accessibility</div></div>
<div class="s" id="s3"><div class="ico sc">\ud83d\udda5\ufe0f</div><h2>Screen Recording</h2><p>Ghost Mode needs to see your screen to find buttons, read text, and navigate visually.</p><div id="sb" class="badge wait">\u23f3 Not granted yet</div><div class="br"><button class="btn g" onclick="openScr()">Open Settings</button><button class="btn p" onclick="nextScr()">I've Enabled It</button></div><div class="hint">System Settings \u2192 Privacy & Security \u2192 Screen Recording</div></div>
<div class="s" id="s4"><div class="orb cel"></div><h1>You're All Set!</h1><div class="cl"><div class="ci">\u2705 Accessibility access</div><div class="ci">\u2705 Screen recording</div></div><p>Press <b>F9</b> anytime to summon Ghost Mode.</p><button class="btn p" onclick="finish()">Launch Ghost Mode</button></div>
<div class="pips" id="pips"><div class="pip on"></div><div class="pip"></div><div class="pip"></div><div class="pip"></div></div>
</div>
<script>
...
async function nextAcc(){await chkAcc();go(3)}
...
async function nextScr(){await chkScr();go(4)}
