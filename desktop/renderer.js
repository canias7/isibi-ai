// ── ISIBI Control Center — Workflow Node Renderer ───────────────────────────
...
function statusClass(s) {
  ...
  if (s === 'generating' || s === 'building') return 'deploying';
...
async function init() {
  ...
  if (token) { showDashboard(); loadApps(); loadUnreadCount(); }
  else showLogin();
...
async function handleLogin() {
  ...
  if (result?.access_token) { showDashboard(); loadApps(); loadUnreadCount(); }
  else { errEl.innerHTML = `<div class="login-error">${result?.detail || result?.error || 'Login failed'}</div>`; btn.disabled = false; btn.textContent = 'Sign In'; }
}
  ...
  document.getElementById('app').innerHTML = `
    ...
    <div class="header">
      ...
      <div class="header-right">
        <button class="btn-icon" id="refresh-btn" title="Refresh"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M21 2v6h-6"/><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M3 22v-6h6"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/></svg></button>
        <button class="btn-icon" id="notif-btn" title="Notifications"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg><span class="badge" id="notif-badge" style="display:none">0</span></button>
        ...
        <svg class="connections-svg" id="connections-svg">
          <defs>
            <linearGradient id="neon-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" style="stop-color:#ec4899;stop-opacity:.5"/>
              <stop offset="50%" style="stop-color:#8b5cf6;stop-opacity:.3"/>
              <stop offset="100%" style="stop-color:#06b6d4;stop-opacity:.5"/>
            </linearGradient>
          </defs>
        </svg>
...
async function loadApps() {
  ...
  if (Array.isArray(result)) {
    apps = result; renderNodes(); renderStats();
    for (const a of apps) loadUptime(a.id);
    requestAnimationFrame(() => setTimeout(drawConnections, 150));
  } else {
    grid.innerHTML = '<div class="empty-state"><p>No apps yet — create one on isibi.ai</p></div>';
  }
}

async function loadUptime(id) {
  const r = await window.isibi.getUptime(id);
  if (r && !r.error) { uptimeData[id] = r; renderNodes(); }
}
async function loadUnreadCount() {
  const r = await window.isibi.getUnreadCount();
  if (r && typeof r.count === 'number') { unreadCount = r.count; updateBadge(); }
}
async function loadNotifications() {
  const r = await window.isibi.getNotifications();
  if (r?.data) { notifications = r.data; renderNotifications(); }
}
...
function drawConnections() {
  ...
  const squares = document.querySelectorAll('.node-square');
  ...
  squares.forEach(sq => {
    ...
    centers.push({ x: r.left - canvasRect.left + r.width / 2, y: r.top - canvasRect.top + r.height / 2 });
  });

  // Connect sequential nodes
  for (let i = 0; i < centers.length - 1; i++) {
    drawLine(svg, centers[i], centers[i + 1], 0.35);
  }
  ...
  // Wrap around
  if (centers.length > 3) {
    drawLine(svg, centers[0], centers[centers.length - 1], 0.08);
  }
}

function drawLine(svg, from, to, opacity) {
  const mx = (from.x + to.x) / 2;
  const my = (from.y + to.y) / 2 - 20 - Math.random() * 20;
  ...
  am.setAttribute('dur', (3 + Math.random() * 5) + 's');
...
function renderStats() {
  ...
  el.innerHTML = `
    <div class="stat-card blue"><div class="stat-value">${total}</div><div class="stat-label">Total Apps</div></div>
...
function renderNodes() {
  const grid = document.getElementById('nodes-grid');
  if (!grid) return;
  if (apps.length === 0) { grid.innerHTML = '<div class="empty-state"><p>No apps yet</p></div>'; return; }
  ...
  grid.innerHTML = apps.map(app => {
    ...
    return `
      <div class="app-node ${cls}" data-id="${app.id}">
        <div class="node-square" style="background:${color}" onclick="openApp('${app.id}')">
        ...
        <div class="node-tooltip">
          ...
          <div class="tt-actions">
            <button class="tt-btn tt-btn-primary" onclick="event.stopPropagation();openApp('${app.id}')">Open</button>
            <button class="tt-btn tt-btn-secondary" onclick="event.stopPropagation();doHealthCheck('${app.id}')" ${il?'disabled':''}>
              ${il==='health'?'<span class="spinner" style="width:10px;height:10px"></span>':'Check'}
            </button>
            <button class="tt-btn tt-btn-warn" onclick="event.stopPropagation();doRestart('${app.id}')" ${il?'disabled':''}>
...
function updateBadge() {
  ...
  if (!b) return;
  if (unreadCount > 0) { b.style.display = 'flex'; b.textContent = unreadCount > 9 ? '9+' : unreadCount; }
...
async function openApp(id) {
  ...
  window.isibi.openExternal(s?.url || `https://isibi-backend.onrender.com/live/${id}`);
}
...
function toggleNotifications() {
  ...
  if (p) { p.classList.toggle('open', notifOpen); if (notifOpen) loadNotifications(); }
}
