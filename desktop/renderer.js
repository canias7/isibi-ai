...
function showDashboard() {
  document.getElementById('app').innerHTML = `
    ...
    <div class="notif-panel" id="notif-panel">
      ...
      <div id="mark-all" style="padding:8px 20px">
...
async function loadApps() {
  ...
  if (Array.isArray(result)) {
    ...
    renderApps();
    renderStats();
    // Load uptime for each app
    for (const app of apps) {
      loadUptime(app.id);
    }
  } else {
    grid.innerHTML = '<div class="empty-state"><div class="empty-icon">!</div><p>Failed to load apps</p></div>';
  }
}
    ...
    return `
      <div class="app-card">
        ...
        <div class="app-actions">
          <button class="btn btn-primary" onclick="openApp('${app.id}')" title="Open in browser">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
            Open
          </button>
          <button class="btn btn-secondary" onclick="doHealthCheck('${app.id}')" ${isLoading ? 'disabled' : ''} title="Run health check">
            ${isLoading === 'health' ? '<span class="spinner"></span>' : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 12h-4l-3 9L9 3l-3 9H2"/></svg>'}
            Check
          </button>
          <button class="btn btn-danger" onclick="doRestart('${app.id}')" ${isLoading ? 'disabled' : ''} title="Restart app">
            ${isLoading === 'restart' ? '<span class="spinner"></span>' : '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 2v6h-6"/><path d="M3 12a9 9 0 0 1 15-6.7L21 8"/><path d="M3 22v-6h6"/><path d="M21 12a9 9 0 0 1-15 6.7L3 16"/></svg>'}
            Restart
          </button>
        </div>
      </div>
...
async function doHealthCheck(projectId) {
  ...
  renderApps();
  const result = await window.isibi.healthCheck(projectId);
  if (result && !result.error) {
    uptimeData[projectId] = { ...uptimeData[projectId], ...result };
  }
