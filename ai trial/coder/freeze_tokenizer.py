# freeze_tokenizer.py — build the BPE tokenizer ONCE, then freeze it forever.
#
# Run this a SINGLE time. After that, never run it again — the vocab is locked,
# which is what lets you add more data later and CONTINUE training (resume)
# instead of starting from scratch every time.

import os, re, json, time, collections

MERGES = int(os.environ.get("MERGES", 2000))
SAMPLE = int(os.environ.get("SAMPLE", 50_000_000))   # learn the tokenizer from up to this many chars

text = open("code.txt", encoding="utf-8").read()[:SAMPLE]
