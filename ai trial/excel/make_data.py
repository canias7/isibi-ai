...
KPI = [
    ...
    # deeper finance / ratios
    ("operating margin from operating income A1 and revenue B1", "=A1/B1"),
    ("asset turnover from revenue A1 and total assets B1", "=A1/B1"),
    ("return on assets from net income A1 and total assets B1", "=A1/B1"),
    ("return on equity from net income A1 and equity B1", "=A1/B1"),
    ("cash ratio from cash A1 and current liabilities B1", "=A1/B1"),
    ("gearing ratio from total debt A1 and equity B1", "=A1/B1"),
    ("working capital from current assets A1 and current liabilities B1", "=A1-B1"),
    # sales / marketing
    ("customer acquisition cost from spend A1 and new customers B1", "=A1/B1"),
    ("lead velocity rate from this month leads A1 and last month B1", "=(A1-B1)/B1"),
    ("MQL conversion rate from MQLs A1 and total leads B1", "=A1/B1"),
    ("upsell rate from upsold customers A1 and total customers B1", "=A1/B1"),
    ("magic number from new ARR A1 and prior quarter sales spend B1", "=A1/B1"),
    ("burn multiple from net burn A1 and net new ARR B1", "=A1/B1"),
    ("rule of 40 from growth rate A1 and profit margin B1", "=A1+B1"),
    # HR
    ("offer to hire ratio from offers A1 and hires B1", "=A1/B1"),
    ("internal mobility rate from internal moves A1 and total hires B1", "=A1/B1"),
    ("billable utilization from billable hours A1 and available hours B1", "=A1/B1"),
    ("realization rate from billed hours A1 and worked hours B1", "=A1/B1"),
    # manufacturing / ops / logistics
    ("rework rate from reworked units A1 and total produced B1", "=A1/B1"),
    ("on-time in-full from perfect orders A1 and total orders B1", "=A1/B1"),
    ("perfect order rate from error-free orders A1 and total B1", "=A1/B1"),
    ("dock to stock time from arrival A1 to shelved B1", "=B1-A1"),
    ("capacity fill rate from booked A1 and capacity B1", "=A1/B1"),
    # healthcare
    ("readmission rate from readmissions A1 and discharges B1", "=A1/B1"),
    ("average length of stay from total patient days A1 and discharges B1", "=A1/B1"),
    ("collection rate from collected A1 and billed B1", "=A1/B1"),
    # retail / ecommerce
    ("cart abandonment rate from abandoned A1 and started checkouts B1", "=A1/B1"),
    ("return rate from returns A1 and units sold B1", "=A1/B1"),
...
CONCEPTS = {
    ...
    "bad debt": "Money owed by customers that you no longer expect to collect. It's written off as an expense once it's clearly uncollectable.",
    "allowance for doubtful accounts": "An estimate of receivables you probably won't collect, set aside in advance so income isn't overstated.",
    "bank reconciliation": "Matching your books to the bank statement to catch missing transactions, fees, or errors so both agree.",
    "chart of accounts": "The organized list of every account (assets, liabilities, income, expenses) used to record transactions.",
    "general ledger": "The master record of all accounts and transactions — everything posts here and the financial statements are built from it.",
    "journal entry": "A single recorded transaction with equal debits and credits, the basic building block of double-entry bookkeeping.",
    "adjusting entries": "End-of-period entries that record accruals, deferrals and depreciation so the books match the period they belong to.",
    "matching principle": "Record expenses in the same period as the revenue they helped generate, so profit reflects true performance.",
    "materiality": "An item is material if leaving it out or getting it wrong would change a reader's decision; tiny immaterial items can be simplified.",
    "going concern": "The assumption that a business will keep operating into the foreseeable future, which underlies how assets are valued.",
    "GAAP vs IFRS": "Two accounting rule sets: GAAP is US-based and more prescriptive; IFRS is international and more principles-based.",
    "fiscal year": "A company's 12-month accounting period, which doesn't have to match the calendar year.",
    "dividends": "Profit distributed to shareholders. They reduce retained earnings and are not an expense on the income statement.",
    "equity vs debt financing": "Equity raises cash by selling ownership (no repayment, but dilution); debt borrows it (must repay with interest, no dilution).",
    "line of credit": "A pre-approved borrowing limit you can draw on as needed and repay, paying interest only on what you use.",
    "factoring": "Selling your receivables to a third party at a discount to get cash now instead of waiting for customers to pay.",
    "free cash flow": "Operating cash flow minus capital expenditures — the cash actually available after keeping the business running.",
    "WACC": "Weighted average cost of capital — the blended cost of a company's debt and equity, used as a hurdle rate for investments.",
    "ROI vs ROE": "ROI measures return on a specific investment; ROE measures net income against shareholders' equity, i.e. return to owners.",
    "liquidity vs solvency": "Liquidity is whether you can cover short-term bills; solvency is whether you can meet long-term obligations and survive.",
    "current vs non-current assets": "Current assets convert to cash within a year (cash, receivables, inventory); non-current ones are long-term (equipment, property).",
    "impairment": "Writing an asset down when its market value drops below book value, recognizing the loss in the period it happens.",
...
CONSULT = [
    ...
    ("how do I improve my cash conversion cycle", "Collect receivables faster, turn inventory quicker, and stretch payables sensibly — the gap between paying out and getting paid is what to shrink."),
    ("my gross margin is fine but net is low, why", "The cost is below the gross line — overhead, salaries, interest or one-offs. Walk down the P&L to find which operating line is heavy."),
    ("should I pay off debt or invest the cash", "Compare the interest rate to your expected return. If debt costs more than you'd reliably earn, pay it down; otherwise invest."),
    ("how do I value my business", "Common rough cuts: a multiple of EBITDA or revenue for your industry, or discounted future cash flows. Get a few angles and compare."),
    ("my biggest customer is 40 percent of revenue, is that a risk", "Yes — concentration risk. If they leave you lose 40% overnight. Diversify the base and lock in longer contracts where you can."),
    ("what is a healthy cash reserve", "A common rule is 3 to 6 months of operating expenses, more if your revenue is lumpy or seasonal."),
    ("how do I forecast next year's revenue", "Start from current run-rate, layer in known wins and churn, apply a realistic growth rate by segment, and keep a conservative case."),
    ("should I offer net 30 terms", "It can win bigger customers but ties up cash. Offer it selectively, check creditworthiness, and price in the delay."),
    ("how do I track project profitability", "Tag revenue and all costs (labor, materials, overhead allocation) to each project, then compare — many 'busy' projects lose money."),
    ("what KPIs should a small business watch", "Cash balance and runway, gross margin, revenue vs last period, AR aging, and customer acquisition cost vs lifetime value."),
    ("how do I set a sales commission structure", "Tie it to margin or collected revenue (not just bookings), keep it simple, and cap or taper only if it threatens unit economics."),
    ("how do I budget for a new hire", "Add salary plus about 25-35% for taxes and benefits, onboarding time, tools, and the ramp before they're fully productive."),
    ("my expenses spiked this month, what should I check", "Compare line by line to last month: one-off purchases, annual renewals hitting at once, duplicate charges, or a timing shift in payables."),
    ("how do I read a P&L", "Top is revenue, subtract COGS for gross profit, subtract operating expenses for operating income, then interest and tax for net income."),
    ("should I factor my receivables", "Only if you genuinely need cash faster and the fee is cheaper than your alternatives — it's expensive financing, not free money."),
    ("how do I price a new product", "Cover cost plus target margin as a floor, then check what the market and competitors bear and what value it delivers — price to value, not just cost."),
...
SCHEMA = {
    ...
    "bug tracker": "Bug ID, Title, Severity, Status, Assignee, Reported, Resolved",
    "sprint backlog": "Story, Points, Priority, Assignee, Sprint, Status",
    "lead list": "Lead, Source, Company, Status, Owner, Last Contact",
    "onboarding checklist": "Task, Owner, Due Date, Status, Notes",
    "meeting action log": "Date, Topic, Owner, Action Item, Due Date, Status",
    "okr tracker": "Objective, Key Result, Target, Current, Progress, Owner",
    "bill of materials": "Component, Quantity, Unit Cost, Total Cost, Supplier",
    "timesheet": "Date, Employee, Task, Start, End, Hours, Billable",
    "price list": "SKU, Product, Cost, Margin, Price, Effective Date",
    "customer feedback log": "Date, Customer, Rating, Comment, Category, Follow Up",
    "inventory count sheet": "SKU, Location, Counted, System Count, Variance",
    "AP aging report": "Vendor, Invoice, Amount, Due Date, Days Overdue, Bucket",
    "AR aging report": "Customer, Invoice, Amount, Due Date, Days Overdue, Bucket",
    "cap table": "Shareholder, Shares, Class, Ownership Percent, Investment",
    "fixed asset schedule": "Asset, Cost, Purchase Date, Useful Life, Depreciation, Book Value",
    "sales forecast": "Month, Product, Units, Price, Forecast Revenue",
    "headcount plan": "Role, Department, Start Date, Salary, Status",
