...
CLAUDE_TOOLS = [
    ...
    {"name": "generate_image", "description": "Generate a new image. YOU must expand the user's brief request into a detailed 50-150 word prompt with subject, setting, lighting, mood, colors, composition, and art style. Never pass a short prompt.", "input_schema": {"type": "object", "properties": {"description": {"type": "string", "description": "Detailed, vivid image prompt (50-150 words) with subject, setting, lighting, mood, colors, composition, art style"}, "size": {"type": "string", "enum": ["1024x1024", "1536x1024", "1024x1536"], "description": "1024x1024 for square/objects, 1536x1024 for landscapes/wide, 1024x1536 for portraits/tall"}}, "required": ["description"]}},
    {"name": "edit_image", "description": "Edit a photo the user sent. Capabilities: remove/change background, style transfer, add/remove objects, color changes, lighting, text overlay, age faces, change clothing, swap day/night, enhance.", "input_schema": {"type": "object", "properties": {"edit_prompt": {"type": "string", "description": "Specific, detailed edit instruction"}}, "required": ["edit_prompt"]}},
...
async def chat_proxy(
    ...
    if tool_use:
        ...
        action_map = {
            ...
            "generate_image": {"type": "generate_image", "target": inp.get("description", ""), "key": inp.get("size", "1024x1024")},
