...
Request: "When an email from my boss arrives, Slack me a one-line summary." (Gmail + Slack connected) → emit_workflow({"title":"Boss Email Alerts","instruction":"When a new email arrives in Gmail, check whether it's from the user's boss. If it isn't, do nothing. If it is, read it and write a one-line summary covering what it's about and whether it needs a reply, then send that as a Slack direct message to the user. Never send more than one DM per email.","trigger":{"type":"event","event":{"app":"gmail","filter":"a new email from the user's boss"}},"nodes":[{"id":"n1","kind":"trigger","app":"event","label":"Boss emails","detail":"New Gmail from the boss"},{"id":"n2","kind":"action","app":"ai","label":"Summarize","detail":"One line: topic + reply needed?"},{"id":"n3","kind":"action","app":"slack","label":"Slack DM","detail":"Send the summary to the user"}],"edges":[{"from":"n1","to":"n2"},{"from":"n2","to":"n3"}]})`;
