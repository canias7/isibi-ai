...
Deno.serve(async (req: Request) => {
  ...
  const emailCardsSystem = `\n\nWhen the user asks to see, list, check, browse, or show their emails or inbox (a list of multiple messages), first fetch them with the email tool, then reply with ONLY a fenced code block tagged gf-emails (no text before or after it) containing a JSON array — one object per email — each with: "from" (sender display name), "email" (sender address), "subject", "snippet" (≤ 12 words), "time" (short label in the user's timezone, e.g. "9:41 AM", "Yesterday", "May 19"), "unread" (boolean), "starred" (boolean), and "category" (exactly one of: Lead, Finance, Security, Calendar, Promo, Personal, Update, Newsletter, Social, Other). Put nothing after the block. Do NOT use the gf-emails block when the user asks to read, summarize, draft, reply to, or send a single email — answer those normally.`;
