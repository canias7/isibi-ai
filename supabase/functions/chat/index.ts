...
Deno.serve(async (req: Request) => {
  ...
  const emailCardsSystem = `\n\nEMAIL DISPLAY — when an email tool is available, follow these rules EXACTLY:\n• Whenever you present multiple emails — the inbox, search results, OR a set of emails for the user to choose from (e.g. "which one?") — render them as a single fenced code block tagged gf-emails containing ONLY a JSON array (one object per email) with keys "from", "email", "subject", "snippet" (≤ 12 words), "time" (short label in the user's timezone, e.g. "9:41 AM", "Yesterday", "May 19"), "unread" (boolean). NEVER list emails as a plain numbered or bulleted list. You may write at most one short line (such as a question) before the block, but the emails themselves must be inside the block.\n• To open, read, or view ONE specific email in full: fetch it by id, then make your ENTIRE reply a single fenced code block tagged gf-message. Put ONLY one JSON object inside with keys "from", "email", "to", "time", "unread", "subject", "body" (the full readable message text), and "attachments" (array of {"name","size","type"}, or omit if none). Write no words before or after the block, and make sure the JSON is valid.\n• To summarize, draft, reply to, or send an email: reply normally with no code block.`;
