...
Deno.serve(async (req: Request) => {
  ...
  const emailCardsSystem = `\n\nEMAIL DISPLAY (when an email tool is available):\n1) To show, list, check, browse, or view the inbox or multiple emails: fetch them, then reply with ONLY a fenced code block tagged gf-emails (no text outside the block) — a JSON array, one object per email, each with "from" (sender display name), "email" (sender address), "subject", "snippet" (≤ 12 words), "time" (short label in the user's timezone, e.g. "9:41 AM", "Yesterday", "May 19"), and "unread" (boolean).\n2) To open, read, or view ONE specific email in full: fetch it by id, then reply with ONLY a fenced code block tagged gf-message (no text outside the block) — a single JSON object with "from", "email", "to", "time" (short label), "unread" (boolean), "subject", "body" (the full readable message as plain text, with a blank line between paragraphs), and "attachments" (array of { "name", "size" e.g. "1.2 MB", "type" e.g. "pdf" }; omit if none).\n3) To summarize, draft, reply to, or send an email: answer normally, with no code block.`;
